...
TEST(ArithmeticExpr, test_value_type)
{
  ...
  ArithmeticExpr         expr_int(ADD, std::move(left_expr), std::move(right_expr));
  ...
  ArithmeticExpr expr_float(ArithOp::ADD, std::move(left_expr), std::move(right_expr));
  ASSERT_EQ(AttrType::FLOATS, expr_float.value_type());

  left_expr.reset(new ValueExpr(int_value1));
  right_expr.reset(new ValueExpr(float_value2));
  ArithmeticExpr expr_int_float(ArithOp::ADD, std::move(left_expr), std::move(right_expr));
  ASSERT_EQ(AttrType::FLOATS, expr_int_float.value_type());

  left_expr.reset(new ValueExpr(int_value1));
  right_expr.reset(new ValueExpr(int_value2));
  ArithmeticExpr expr_int_int(ArithOp::DIV, std::move(left_expr), std::move(right_expr));
...
TEST(ArithmeticExpr, test_try_get_value)
{
  ...
  ArithmeticExpr        int_expr(ArithOp::ADD, std::move(left_expr), std::move(right_expr));
  ...
  left_expr.reset(new ValueExpr(int_value1));
  right_expr.reset(new ValueExpr(int_value2));
  new (&int_expr)(ArithmeticExpr)(ArithOp::SUB, std::move(left_expr), std::move(right_expr));
  ASSERT_EQ(int_expr.try_get_value(int_result), RC::SUCCESS);
  ...
  right_expr.reset(new ValueExpr(int_value2));
  int_expr.~ArithmeticExpr();
  new (&int_expr)(ArithmeticExpr)(ArithOp::MUL, std::move(left_expr), std::move(right_expr));
  ASSERT_EQ(int_expr.try_get_value(int_result), RC::SUCCESS);
  ...
  right_expr.reset(new ValueExpr(int_value2));
  int_expr.~ArithmeticExpr();
  new (&int_expr)(ArithmeticExpr)(ArithOp::DIV, std::move(left_expr), std::move(right_expr));
  ASSERT_EQ(int_expr.try_get_value(float_result), RC::SUCCESS);
  EXPECT_FLOAT_EQ(float_result.get_float(), 0.5);

  left_expr.reset(new ValueExpr(float_value1));
  right_expr.reset(new ValueExpr(float_value2));
  ArithmeticExpr float_expr(ArithOp::ADD, std::move(left_expr), std::move(right_expr));
  ...
  new (&float_expr)(ArithmeticExpr)(ArithOp::SUB, std::move(left_expr), std::move(right_expr));
  ASSERT_EQ(float_expr.try_get_value(float_result), RC::SUCCESS);
  ...
  right_expr.reset(new ValueExpr(float_value2));
  float_expr.~ArithmeticExpr();
  new (&float_expr)(ArithmeticExpr)(ArithOp::MUL, std::move(left_expr), std::move(right_expr));
  ASSERT_EQ(float_expr.try_get_value(float_result), RC::SUCCESS);
  ...
  right_expr.reset(new ValueExpr(float_value2));
  float_expr.~ArithmeticExpr();
  new (&float_expr)(ArithmeticExpr)(ArithOp::DIV, std::move(left_expr), std::move(right_expr));
  ASSERT_EQ(float_expr.try_get_value(float_result), RC::SUCCESS);
  EXPECT_FLOAT_EQ(float_result.get_float(), 1.1 / 2.2);
  ...
  ArithmeticExpr zero_int_expr(ArithOp::DIV, std::move(left_expr), std::move(right_expr));
  ASSERT_EQ(zero_int_expr.try_get_value(float_result), RC::SUCCESS);

  left_expr.reset(new ValueExpr(float_value1));
  right_expr.reset(new ValueExpr(zero_float_value));
  ArithmeticExpr zero_float_expr(ArithOp::DIV, std::move(left_expr), std::move(right_expr));
  ASSERT_EQ(zero_float_expr.try_get_value(float_result), RC::SUCCESS);

  left_expr.reset(new ValueExpr(int_value1));
  ArithmeticExpr negative_expr(ArithOp::NEGATIVE, std::move(left_expr), nullptr);
  ASSERT_EQ(negative_expr.try_get_value(int_result), RC::SUCCESS);
  ASSERT_EQ(int_result.get_int(), -1);

  left_expr.reset(new ValueExpr(float_value1));
  ArithmeticExpr negative_float_expr(ArithOp::NEGATIVE, std::move(left_expr), nullptr);
